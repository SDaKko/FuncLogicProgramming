...
compiled(java,1).    % Компилируемый в байт-код
compiled(javascript,0). %
compiled(ruby,0).  
compiled(c_sharp,1).
compiled(python,0).   
compiled(c_plu_plus,1). 
compiled(f_sharp,1).   
compiled(prolog,0).     
compiled(c,1).   
compiled(asm,1).        
...
question8(X8):- write("Does your language compile to bytecode for a virtual machine?"),nl,
                write("1. Yes"),nl,
                write("0. NO"),nl,
                read(X8).				
...
pr:-	question1(X1),question2(X2),question3(X3),question4(X4),
		...
		cross(X,X5),visual(X,X6),mobile(X,X7), compiled(X, X8),
		write(X).
